...
顾名思义就是在节点的外部插入新的节点。与内部插入不同的是，外部插入的新节点和目标节点的关系是兄弟关系而不是父子关系。

当然，和内部插入一样，jQuery也为外部插入提供了一系列方法，即after()，insertAfter()，before()和insertBefore()。

**包裹操作**

包裹节点的意思就是将某个或者一组符合条件的节点用其他元素标记包裹起来。一般来说，这些元素标记都是结构化的HTML标记，它并不能改变或破坏原始节点的内容，只是起到改变节点样式的效果。
